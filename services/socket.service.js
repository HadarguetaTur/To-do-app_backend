...
function connectSockets(http, session) {
  ...
  gIo.on("connection", (socket) => {
    ...
    socket.on("task-added", async (task) => {
      console.log("task has added!", task)
      gIo.emit("task-added", task)
    })
    socket.on("task-removed", async (task) => {
      console.log("task has removed!", task)
      gIo.emit("task-removed", task)
    })
    socket.on("task-filter", async (task) => {
      gIo.emit("task-filter", task)
    })
